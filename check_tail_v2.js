...
const targets = $('h2');
let output = `Total Targets: ${targets.length}\n`;
...
if (lastMarker) {
    ...
    output += `Tail Size: ${Buffer.byteLength(tailXml, 'utf8')} bytes\n`;
} else {
    output += `Last marker index ${lastIdx} not found!\n`;
}

fs.writeFileSync('tail_audit_utf8.txt', output, 'utf8');
